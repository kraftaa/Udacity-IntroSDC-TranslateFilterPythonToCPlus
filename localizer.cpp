                               ...
                               float p_miss)
{
    ...
    vector<vector<float> > newGrid;//(height, std::vector<float> (width, 0)); //-if I do this then after compiling and running the file it's crashing
    vector<float> newRow;
    for (int i = 0; i < height; i++) {
        newRow.clear();
        for (int j = 0; j < width; j++) {
            if (grid[i][j] == color) {
//                newGrid[i][j] = (beliefs[i][j] * p_hit); //if I do this then after compiling and running the file it's crashing
                newRow.emplace_back(beliefs[i][j] * p_hit);
            } else {
//                newGrid[i][j] = (beliefs[i][j] * p_miss);
                newRow.emplace_back(beliefs[i][j] * p_miss);
            }
        }
